...
if __name__ == "__main__":
    # Define directory
    text_directory = "data/texts_from_pdfs/"
    raw_input_directory = "data/pdf_collections/"

    os.makedirs(raw_input_directory, exist_ok=True)
    os.makedirs(text_directory, exist_ok=True)

    # List of Git repository URLs to clone
    git_repositories = []
    with open('data/pdf_collections/git_repo_collections.txt') as  f:
    ...
    cloned_dirs = clone_repositories(git_repositories, raw_input_directory)
    print(f"Cloned {len(cloned_dirs)} repos")
    
    # Traverse the directories and extract text from PDFs
    traverse_and_extract(raw_input_directory, text_directory)
